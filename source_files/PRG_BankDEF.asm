...
LA57A:  JSR $AF02
LA57D:  LDA FightVar00      ;($00) Fight status flag?
LA57F:  CMP #$01
LA581:  BNE $A57A
LA583:  LDA #$02
LA585:  JSR $AF04
...
LA58E:  STA $22
LA590:  JSR $AF02
LA593:  LDX FightVar00      ;($00) Fight status flag?
LA595:  BPL $A590
LA597:  JSR $BF7E
LA59A:  JSR $A750
LA59D:  JSR $AA1D
...
LA8ED:  JSR $AF02
LA8F0:  LDA FightVar00      ;($00) Fight status flag?
LA8F2:  CMP #$01
LA8F4:  BNE $A8ED
LA8F6:  LDA #$02
LA8F8:  JSR $AF04
...
LA909:  STA $22
LA90B:  JSR $AF02
LA90E:  LDX FightVar00      ;($00) Fight status flag?
LA910:  BPL $A90B
LA912:  JSR $BF7E
LA915:  JSR $AA1D
LA918:  LDA #$FF
...
LA935:  LDA #$20
LA937:  JSR $AF04
LA93A:  LDA FightVar09        ;($09)
LA93C:  STA FightVar08        ;($08)
LA93E:  LDX FightVar00      ;($00) Fight status flag?
LA940:  INX
LA941:  BEQ $A9AA
LA943:  INX
LA944:  BEQ $A977
...
LA981:  LDX #$01
LA983:  JSR $AE28
LA986:  LDA FightVar09        ;($09)
LA988:  BEQ $A996
LA98A:  BMI $A9C5
LA98C:  AND #$0F
LA98E:  BEQ $A9C1
LA990:  LDA FightVar08        ;($08)
LA992:  AND #$0F
LA994:  BNE $A96F
LA996:  LDA #$00
LA998:  STA $03C0
...
LAC2D:  LDA #$30
LAC2F:  STA FightVar00      ;($00) Fight status flag?
LAC31:  LDA #$01
LAC33:  STA $30
LAC35:  STA $38
LAC37:  RTS
...
LACBF:  LDX #$01
LACC1:  LDA FightVar00      ;($00) Fight status flag?
LACC3:  AND #$01
LACC5:  BNE $ACCF
LACC7:  LDA FightVar08        ;($08)
LACC9:  AND #$0F
LACCB:  BNE $ACCF
LACCD:  LDX #$05
LACCF:  JSR $ADD1
...
LAE2E:  BNE $AE3D
LAE30:  LDA $A10D,Y
LAE33:  STA FightVar01
LAE35:  JSR $AE89
LAE38:  LDX #$00
LAE3A:  STX MacLosses          ;($0A)
LAE3C:  RTS

LAE3D:  LDA MacLosses          ;($0A)
LAE3F:  BEQ $AE4B
LAE41:  LDA $A10F,Y
LAE44:  BMI $AE3C
LAE46:  LDA $A10E,Y
...
LAE89:  LDY FightVar01
LAE8B:  LDA $A18D,Y
LAE8E:  STA FightVar09        ;($09)
LAE90:  RTS
...
LB530:  LDA FightVar00      ;($00) Fight status flag?
LB532:  BMI $B52F
LB534:  AND #$0F
LB536:  TAX
LB537:  LDA FightVar00      ;($00) Fight status flag?
...
LB581:  LDA #$40
LB583:  STA FightVar00      ;($00) Fight status flag?
LB585:  RTS
...
LB5AF:  CMP #$04                ;If RoundNumber < 4, then -->
LB5B1:  BCC $B5B6               ; skip the following JMP -->
LB5B3:  JMP $B621               ; else skip to the final portion of this function
LB5B6:  ASL
LB5B7:  ADC RoundNumber         ;X = 3*RoundNumber
LB5B9:  TAX
LB5BA:  CMP #$03                ;If RoundNumber==1, then -->
LB5BC:  BEQ $B5CB               ; skip to setting the upper digit for hearts
LB5BE:  LDA HeartTable,X        ;Get initial hearts for this round
LB5C1:  JSR Div16
LB5C4:  CMP CurHeartsUD         ;If upper digit already matches current, then -->
LB5C7:  BEQ $B5DF               ; skip to checking lower digit
LB5C9:  BCC $B5EC               ;If lower digit less than current, then leave it alone
LB5CB:  LDA HeartTable,X
LB5CE:  JSR Div16
LB5D1:  STA CurHeartsUD         ;Set UD of current hearts to initial value
LB5D4:  LDA HeartTable,X
LB5D7:  AND #LO_NIBBLE
LB5D9:  STA CurHeartsLD         ;Set LD of current hearts to initial value
LB5DC:  JMP $B5EC               ;Skip checking the lower digit
LB5DF:  LDA HeartTable,X
LB5E2:  AND #LO_NIBBLE
LB5E4:  CMP CurHeartsLD         ;If lower digit is less than initial, then -->
LB5E7:  BCC $B5EC               ;  do not modify the lower digit -->
LB5E9:  STA CurHeartsLD         ;  else set LD of current hearts to initial value
LB5EC:  INX
LB5ED:  LDY #$00                ;For Loop Y=0; Y<4; Y+=2
LB5EF:  LDA HeartTable,X
LB5F2:  JSR Div16
LB5F5:  STA HeartRecover,Y      ;Set the upper digit of heart recovery for this round
LB5F8:  INY
LB5F9:  LDA HeartTable,X
LB5FC:  AND #LO_NIBBLE
LB5FE:  STA HeartRecover,Y      ;Set the upper digit of heart recovery for this round
LB601:  INY
LB602:  INX
LB603:  CPY #$04
LB605:  BNE $B5EF               ;End For Loop
...
LB9D8:  LDA #$01
LB9DA:  STA $17
LB9DC:  LDX FightVar00      ;($00) Fight status flag?
LB9DE:  INX
LB9DF:  INX
LB9E0:  BEQ $BA0C
LB9E2:  INX
...
LBB2A:  LDY #$7D
LBB2C:  JSR $AF2E
LBB2F:  LDA FightVar09        ;($09)
LBB31:  AND #$0F
LBB33:  BNE $BB4A
LBB35:  LDA #$53
LBB37:  LDX $013E
LBB3A:  BNE $BB44
LBB3C:  LDA FightVar09        ;($09)
